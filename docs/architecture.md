...
76 automated tests exist: 69 inline `#[cfg(test)]` unit tests, and 7 integration tests in `tests/` that run the real binary. All run under a plain `cargo test`. There is no CI configuration for rproj itself.
