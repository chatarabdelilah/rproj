...
- **CI has been observed passing, not observed failing.** The green run proves the workflow executes; it does not prove the gate rejects bad code, since that path has only ever been exercised locally (§9's gate entry). Pushing a deliberately broken spec once would close it.
