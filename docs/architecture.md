...
36 automated tests exist, all inline `#[cfg(test)]` unit tests run by `cargo test`. There is no CI configuration for rproj itself and `Cargo.toml` declares no dev-dependencies (nothing beyond `std` assertions is needed).
...
| `src/steps/testez.rs` (3 tests) | TestEZ Companion roots are service-rooted (not `game/`-prefixed) and name the same lowercase instances the project file creates — a mismatched root finds no tests, which is indistinguishable from every test passing; and the bundled `testez.yml` declares every global selene would otherwise reject. |
...
| `src/catalog/tool_settings.rs` (5 tests) | The scaffolded config and `rproj configure`'s defaults render identically; StyLua's scaffolded config selects `Luau` syntax; Selene's `std` is overridable for TestEZ; top-level TOML keys precede any `[table]` header; every configurable tool is findable by key. |
