...
| `lute` | `gate` |
| `tarmac` | `asset-pipeline` / `asphalt` (legacy compatibility only; Tarmac is no longer supported) |
| `asphalt` | `asset-pipeline` / `asphalt` |
| `tungsten` | `asset-pipeline` / `tungsten` |
...
- **`asset-pipeline` owns both `figma/` and exactly one config file**, because they are one pipeline — `figma/exports/` is where designs land and the chosen config is what uploads or syncs them. The old `assets-2d` key is accepted as a legacy alias and re-emitted as `asset-pipeline`.
...
| `src/catalog/mod.rs` (7 tests) | Catalog integrity: keys are unique across tools *and* packages (a duplicate makes `find` return one and orphan the other); every tool's `family` appears in `FAMILY_ORDER` (a missing one never prints in `rproj info`); companion rules name real packages; every entry has a non-empty description and an `https://` docs URL; every package `source` parses as `scope/name@version`; usage notes key onto something that exists; removed external tools stay out of the supported catalog. |
