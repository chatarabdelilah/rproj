...
60 automated tests exist, all inline `#[cfg(test)]` unit tests run by `cargo test`. There is no CI configuration for rproj itself and `Cargo.toml` declares no dev-dependencies (nothing beyond `std` assertions is needed).
...
- ~~No test covers `commands::configure`.~~ **Mostly closed.** Everything the command decides — reading current values, merging, ordering, the no-op property — now lives in `catalog::tool_settings` under test (§9's table). What remains untested is the thin shell around it: the two `ConfigTarget` writers' file I/O and the prompt rendering itself, verified instead by driving all four tools end-to-end through a ConPTY harness (below).
...
- **Nothing exercises the interactive pickers automatically**, but they are no longer unverified. `inquire` reads the Windows console input handle directly and ignores piped stdin — a piped `rproj configure stylua` renders its first prompt and then hangs forever — so driving them needs a real pseudo-terminal. A throwaway `portable-pty` harness (open a ConPTY, spawn `rproj` into it, write key sequences like `down,down,enter` / `type:80,enter` on a timer, capture the screen) was used to run `rproj new` through the mode picker and package multi-select, and `rproj configure` through all four tools, both `ConfigTarget` writers, all three `SettingKind` prompts, the no-arg tool picker, the unknown-key error and both unparseable-file refusals. That is what found the two configure bugs in §7. The harness lives outside the repo (it needs a dev-dependency this crate otherwise has none of); reproducing it is ~80 lines. **A `MultiSelect` cannot be driven by arrow keys alone** — `space` toggles, `enter` confirms, and `→`/`←` are select-all/none.
...
- [ ] `rproj watch`, run against a project scaffolded by this version of `rproj new`, has not yet been manually re-verified after the `default.project.json`/sourcemap-ordering changes in §7. (Re-verified for the Wally workflow via `steps::wally::sync`; the git-submodule path has not been re-run since.)
