...
- [ ] **Drop the hand-built `wally-package-types` as soon as upstream cuts a release past `1.6.2`** (see §7 for the bug). The development machine currently has a source build of commit `3fe0a5d` (PR #28) copied over `~/.rokit/tool-storage/johnnymorganz/wally-package-types/1.6.2/wally-package-types.exe`, with the genuine upstream binary kept beside it as `wally-package-types.exe.upstream-1.6.2.bak`. Once a real release contains the fix, restore or re-download the stock binary and let rokit resolve the new version normally. Check with `curl -s https://api.github.com/repos/JohnnyMorganz/wally-package-types/releases/latest`.
