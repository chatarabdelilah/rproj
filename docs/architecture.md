...
`rproj configure project` adds a machine-wide user layer without changing that compiled fallback. It edits `<config>/templates/default.project.json`, starting from the complete built-in document. The editor prompt uses `VISUAL`, then `EDITOR`, then Notepad on Windows. The draft stays in memory while strict JSON, ownership rules, and Rojo sourcemap plus build checks run against all eight reachable combinations of dependency workflow, tests, and server packages. A successful candidate is written to a temporary sibling, synced, and atomically moved over the stored file, so validation or write failures leave the last valid template intact.
...
**The rule this gives for free: an implementation prompt appears only when a capability has more than one implementation.** Same rule as everywhere else — never ask a question with one answer — and it says exactly when a new prompt is permitted. Today `asset-pipeline` qualifies; `test` becomes the second when jest-lua lands, which is why that milestone is one catalog entry rather than a new gate step.
...
212 non-live tests are discovered by a plain `cargo test`: 193 unit tests and 19 integration tests in `tests/` that drive the real binary. The upstream-badge check and two real-Rojo template checks are ignored in the ordinary suite, so 209 run locally. A further 7 live in `tests/live.rs` and are `#[ignore]`d — see below. rproj's own CI (`.github/workflows/ci.yml`) runs the suite on Windows against stable and 1.89 with `--locked`, with clippy and `cargo fmt --all --check` on stable only. A `package` job builds from the published tarball, and a weekly `badges` job on ubuntu runs the maintenance-badge freshness gate authenticated (§3).
...
| `src/steps/rojo.rs` (9 tests) | The built-in document keeps the established source/place shape; custom nodes and top-level settings survive while the real project name and workflow mounts are injected; core mounts cannot move; every dynamic mount name is reserved; arbitrary and conditionally generated `$path` targets are rejected; and Wally/submodule outputs receive only their own dependency mount. The ignored real-tool tests materialize and validate all eight reachable mount combinations through Rojo, and prove an invalid Roblox property value is rejected. |
...
- ~~No test covers any step that shells out or touches the network.~~ **Closed** by `tests/live.rs`, an `#[ignore]`d suite of seven tests run with `cargo test --test live -- --ignored --test-threads=1` (69s). Three project-choice tests avoid network and toolchain work once rproj's machine setup is already recorded; on an unprovisioned machine they stop at that prerequisite instead of silently altering machine-level selections. The remaining four exercise the Wally chain, submodule mounts, the generated gate in both directions, and fresh-clone recovery. `--test-threads=1` is mandatory because the tests share rokit's global manifest and Wally's package cache. What remains uncovered: `winget`, `code` and `blender`, which install software machine-wide and have no business running in a test.
...
| `inquire` | Interactive list/settings prompts and the external-editor round trip | Provides arrow-key prompts, filtering, custom answer formatters, and an editor prompt that honors standard editor variables with a Windows fallback. |
| `tempfile` | Atomic project-template replacement and inquire's editor draft | The validated candidate is staged beside the saved template, synced, then atomically persisted so a failed write cannot truncate the last valid configuration. |
...
- [ ] **The live suite is the only thing that drives `rproj new`'s prompts, and it is `#[ignore]`d** — so a change to the prompt sequence breaks it without failing anything (§9). Re-run `cargo test --test live -- --ignored --test-threads=1` after changes to `commands::new` on a provisioned development machine. The suite currently has no isolated machine-configuration fixture, so an unprovisioned host stops at the setup prompt before reaching project choices.
