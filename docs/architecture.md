...
79 automated tests exist: 72 inline `#[cfg(test)]` unit tests, and 7 integration tests in `tests/` that run the real binary. All run under a plain `cargo test`. There is no CI configuration for rproj itself.
...
| `src/steps/vscode.rs` (8 tests) | The vendored-code globs extend luau-lsp's defaults rather than replacing them, so Wally's `_Index` stays covered for projects using both workflows, and they stay submodule-only since luau-lsp already ignores `_Index` itself. Plus the §7 editor-settings landmines: **both** workflows get settings written (this used to write nothing at all unless the project used submodules); the Studio-plugin bridge is on, without which nothing made in Studio is ever known to autocomplete; a stale machine-wide `stylua.configPath` is neutralised; sourcemap autogeneration is left to `rproj watch` rather than run twice; new files are created with Unix line endings, without which `stylua --check` reports a whole-file diff whose two sides are identical; and the deprecated `luau-lsp.types.roblox` spelling is not written. |
| `src/ui.rs` (7 tests) | Option matching uses the full `key - ` prefix, so `react` doesn't match `reactRoblox`; `option_key` survives labels with no separator; the multi-select help mentions enter-to-confirm. Plus the §2.4 rule itself: a tally reports each kind on one line rather than one line each, an already-present list past four collapses to a count, an empty tally prints nothing at all, and a multi-select summary lists keys rather than echoing every option's full description. `Tally::finish` delegates to a pure `summary()` so those rules are testable without capturing stdout. |
...
- **No test covers any step that shells out** (`winget`, `rokit`, `git`, `rojo`, `code`, `blender`) or touches the network. This is the biggest remaining gap and it is the one that keeps producing §7 entries — the submodule fresh-clone bug is the most recent. The pty harness in `tests/common/` would drive these too; what stops it is that each would need real tools installed, network access, and minutes rather than milliseconds. If it gets built, it wants to be a separate `#[ignore]`d suite run deliberately, not part of `cargo test`.
