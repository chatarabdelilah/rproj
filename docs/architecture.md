...
|---|---|---|
| `ProjectToml { filename }` | `tool_settings::current_toml_values` parses the existing file, including unsupported shapes; catalog defaults only fill in missing settings | Changed answers use the existing line writer, guarded by `configure::checked_toml_merge`. Both the current file and candidate must parse, and the candidate must equal the current parsed document with only the answered values changed. Unusual layouts that the line writer cannot handle are refused before writing. |
