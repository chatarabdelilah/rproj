...
94 automated tests exist: 80 inline `#[cfg(test)]` unit tests, and 14 integration tests in `tests/` that run the real binary. All run under a plain `cargo test`. There is no CI configuration for rproj itself.
