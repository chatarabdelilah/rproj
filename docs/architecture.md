...
44 automated tests exist, all inline `#[cfg(test)]` unit tests run by `cargo test`. There is no CI configuration for rproj itself and `Cargo.toml` declares no dev-dependencies (nothing beyond `std` assertions is needed).
