...
- Formatting, clippy, and locked packaging passed (94 packaged files, 1.1 MiB uncompressed, 294.2 KiB compressed before review fixes). CodeRabbit reviewed the full branch and identified four applicable findings: completion lost after Back/Exit, CRLF doubling, mismatched Rokit trust guidance, and stale architecture documentation. All four are addressed; the focused follow-up found only a module-map indentation issue, now corrected. Final formatting, clippy, and the ordinary suite passed after the runtime fixes. Reviewed-head and merged-main CI remain pending; this is not publication authorization.
