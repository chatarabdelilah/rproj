...
Jest CI runs quality checks by default and explicitly reports cloud tests disabled. Local testing needs no cloud credentials. To enable cloud tests, set repository variable `JEST_OPEN_CLOUD=true` and add repository secret `ROBLOX_OPEN_CLOUD_API_KEY` and repository variables `ROBLOX_UNIVERSE_ID` and `ROBLOX_PLACE_ID`. The API key needs `universe-places:write`, `universe.place.luau-execution-session:write`, and `memory-store.sorted-map:read`/`memory-store.sorted-map:write`; the last pair carries streamed GitHub Actions results. Once enabled, CI fails with the missing names instead of silently skipping tests. Use a dedicated test place: the runner uploads its test build to the configured place. Queue scopes are unnecessary for rproj's generated single-session CI. Workspace or parallel sharding modes passed through `rproj test [args]` additionally require `memory-store.queue:add`, `memory-store.queue:dequeue`, and `memory-store.queue:discard`.
